...
def prepare_outputs(tags, max_len):
    ...
    for x in range(len(tags)):
        if len(tags[x]) > 62:
            continue
        outputs[x][0] = BOS
        outputs[x][len(tags[x]) + 1] = EOS
        for y in range(max_len):
            if y < len(tags[x]):
                outputs[x][y + 1] = tag_to_id[tags[x][y]]
    return outputs
...
#
#
# def convert_to_tensor(data: List[List[str]], max_len: int) -> torch.tensor:
#     """
#
#     :param max_len:
#     :param data:
...
#     """
#
#     data_tensor = torch.zeros(len(data), max_len, dtype=torch.int)
#     sentence_index = 0
#     for i, sentence in enumerate(data):
#         if sentence:  # Check if not empty
#             for j, char in enumerate(sentence):
#                 data_tensor[i, j] = ord(char)
#
#     return data_tensor
#
#
